...
--update scripts for reading 2015-2019 PUMS seed tables because some field names changed/did not exist YS 04/12/2021
...
INTO [hhtable]
FROM
	[psam_h11] -- 2019 ACS PUMS for Chattanooga MPO
...
LEFT OUTER JOIN ( 
	SELECT
		...
		,COUNT(*) AS [nwrkrs_esr]
	FROM
		[psam_p11] -- 2019 ACS PUMS for Chattanooga MPO
	WHERE 
		[ESR] IN (1,2,4,5)
	GROUP BY 
		[SERIALNO]
...
SELECT [PUMA]
	...
	,CASE	WHEN [ESR] NOT IN (1,2,4,5) OR [ESR] IS NULL THEN '999'
			...
			END AS [soc]
FROM 
	[psam_p11] -- 2019 ACS PUMS for rogue valley
) AS tt
